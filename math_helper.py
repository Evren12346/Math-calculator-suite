        ...
        while True:
            ...
            if choice == '1':
                try:
                    ...
                    m1 = Matrix(ast.literal_eval(m1_input))
                    m2 = Matrix(ast.literal_eval(m2_input))
                    result = m1 + m2
                ...
                try:
                    ...
                    m = Matrix(ast.literal_eval(m_input))
                    det = m.det()
...
def logarithm_calculator():
    """Run the logarithm calculator mode."""
    print("=" * 70)
    ...
    try:
        while True:
            print("Options:")
            print("  1) log base N  (log_b(x))")
            print("  2) Natural logarithm (ln)")
            print("  3) Common logarithm (log10)")
            print()

            choice = input("Select option (1-3, or 'quit' to exit): ").strip()

            if choice.lower() == 'quit':
                print("\nExiting logarithm calculator...\n")
                break

            if choice == '1':
                try:
                    x = float(input("Enter value (x): ").strip())
                    b = float(input("Enter base (b): ").strip())
                    if x <= 0:
...
def prime_factorization():
    """Run the prime factorization mode."""
    print("=" * 70)
    ...
    try:
        while True:
            user_input = input("Enter a positive integer >= 2 (or 'quit' to exit): ").strip()

            if user_input.lower() == 'quit':
                print("\nExiting prime factorization...\n")
                break
...
def percentage_calculator():
    """Run the percentage calculator mode."""
    print("=" * 70)
    ...
    try:
        while True:
            print("Options:")
            print("  1) X% of Y")
            print("  2) What percent is X of Y?")
            ...
            if choice.lower() == 'quit':
                print("\nExiting percentage calculator...\n")
                break

            if choice == '1':
                try:
                    pct = float(input("Enter percentage (X): ").strip())
                    y = float(input("Enter value (Y): ").strip())
                    result = (pct / 100) * y
                    print(f"\n✓ {pct}% of {y} = {result}\n")
                except ValueError:
                    print("Error: Please enter valid numbers\n")
                ...
                try:
                    x = float(input("Enter original value (X): ").strip())
                    y = float(input("Enter new value (Y): ").strip())
                    if x == 0:
                        print("Error: Original value cannot be zero\n")
                        continue
                    change = ((y - x) / abs(x)) * 100
                    direction = "increase" if change >= 0 else "decrease"
                    print(f"\n✓ Percentage {direction}: {abs(change):.4f}%\n")
                except ValueError:
                    print("Error: Please enter valid numbers\n")
                ...
                try:
                    y = float(input("Enter value (Y): ").strip())
                    pct = float(input("Enter percentage to increase/decrease (+/-): ").strip())
                    result = y * (1 + pct / 100)
                    direction = "increased" if pct >= 0 else "decreased"
                    print(f"\n✓ {y} {direction} by {abs(pct)}% = {result}\n")
                except ValueError:
                    print("Error: Please enter valid numbers\n")
...
def polynomial_operations():
    """Run the polynomial expand/factor operations mode."""
    print("=" * 70)
    ...
    try:
        while True:
            print("Options:")
            print("  1) Expand polynomial")
            print("  2) Factor polynomial")
            print()

            choice = input("Select option (1-2, or 'quit' to exit): ").strip()
            ...
            expression = input("Enter polynomial expression: ").strip()
